from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from brp.bewoning_api.models.base_model import Model
import re
from brp.bewoning_api import util

import re  # noqa: E501

class InvalidParam(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, type=None, name=None, code=None, reason=None):  # noqa: E501
        """InvalidParam - a model defined in OpenAPI

        :param type: The type of this InvalidParam.  # noqa: E501
        :type type: str
        :param name: The name of this InvalidParam.  # noqa: E501
        :type name: str
        :param code: The code of this InvalidParam.  # noqa: E501
        :type code: str
        :param reason: The reason of this InvalidParam.  # noqa: E501
        :type reason: str
        """
        self.openapi_types = {
            'type': str,
            'name': str,
            'code': str,
            'reason': str
        }

        self.attribute_map = {
            'type': 'type',
            'name': 'name',
            'code': 'code',
            'reason': 'reason'
        }

        self._type = type
        self._name = name
        self._code = code
        self._reason = reason

    @classmethod
    def from_dict(cls, dikt) -> 'InvalidParam':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The InvalidParam of this InvalidParam.  # noqa: E501
        :rtype: InvalidParam
        """
        return util.deserialize_model(dikt, cls)

    @property
    def type(self) -> str:
        """Gets the type of this InvalidParam.


        :return: The type of this InvalidParam.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type: str):
        """Sets the type of this InvalidParam.


        :param type: The type of this InvalidParam.
        :type type: str
        """

        self._type = type

    @property
    def name(self) -> str:
        """Gets the name of this InvalidParam.

        Naam van de parameter  # noqa: E501

        :return: The name of this InvalidParam.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name: str):
        """Sets the name of this InvalidParam.

        Naam van de parameter  # noqa: E501

        :param name: The name of this InvalidParam.
        :type name: str
        """
        if name is not None and not re.search(r'^[a-zA-Z0-9\._]{1,30}$', name):  # noqa: E501
            raise ValueError(r"Invalid value for `name`, must be a follow pattern or equal to `/^[a-zA-Z0-9\._]{1,30}$/`")  # noqa: E501

        self._name = name

    @property
    def code(self) -> str:
        """Gets the code of this InvalidParam.

        Systeemcode die het type fout aangeeft  # noqa: E501

        :return: The code of this InvalidParam.
        :rtype: str
        """
        return self._code

    @code.setter
    def code(self, code: str):
        """Sets the code of this InvalidParam.

        Systeemcode die het type fout aangeeft  # noqa: E501

        :param code: The code of this InvalidParam.
        :type code: str
        """
        if code is not None and not re.search(r'^[a-zA-Z0-9]{1,25}$', code):  # noqa: E501
            raise ValueError(r"Invalid value for `code`, must be a follow pattern or equal to `/^[a-zA-Z0-9]{1,25}$/`")  # noqa: E501

        self._code = code

    @property
    def reason(self) -> str:
        """Gets the reason of this InvalidParam.

        Beschrijving van de fout op de parameterwaarde  # noqa: E501

        :return: The reason of this InvalidParam.
        :rtype: str
        """
        return self._reason

    @reason.setter
    def reason(self, reason: str):
        """Sets the reason of this InvalidParam.

        Beschrijving van de fout op de parameterwaarde  # noqa: E501

        :param reason: The reason of this InvalidParam.
        :type reason: str
        """
        if reason is not None and not re.search(r'^[a-zA-Z0-9\., ]{1,80}$', reason):  # noqa: E501
            raise ValueError(r"Invalid value for `reason`, must be a follow pattern or equal to `/^[a-zA-Z0-9\., ]{1,80}$/`")  # noqa: E501

        self._reason = reason
